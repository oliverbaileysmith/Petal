...
namespace ptl
{
	OpenGLShader::OpenGLShader(const std::string &filepath)
	{
		...
		size_t count = lastDot == std::string::npos ? filepath.size() - start
													: lastDot - start;
	...
	void OpenGLShader::UploadUniformInt(const std::string &name,
		int32_t value) const
	{
		int loc = glGetUniformLocation(m_ID, name.c_str());
		PTL_CORE_ASSERT(loc != -1, "Invalid uniform location");
	...
	void OpenGLShader::UploadUniformInt2(const std::string &name,
	...
	void OpenGLShader::UploadUniformInt3(const std::string &name,
	...
	void OpenGLShader::UploadUniformInt4(const std::string &name,
	...
	void OpenGLShader::UploadUniformUint(const std::string &name,
	...
	void OpenGLShader::UploadUniformUint2(const std::string &name,
	...
	void OpenGLShader::UploadUniformUint3(const std::string &name,
	...
	void OpenGLShader::UploadUniformUint4(const std::string &name,
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::SplitShaders(
		const std::string &source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			// Set pointer to word after #type (this doesn't account for
			// multiple whitespace)
			size_t begin = pos + typeTokenLength + 1;

			// Ensure valid shader type
			std::string type = source.substr(begin, eol - begin);
			PTL_CORE_ASSERT(ShaderTypeFromString(type),
	...
	{
		PTL_CORE_ASSERT(shaderSources.size() < 3,
			"More than 2 shaders in file");
