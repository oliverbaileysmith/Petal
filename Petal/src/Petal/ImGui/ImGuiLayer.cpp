...
namespace ptl
{
    ...
    {
        ImGuiIO& io = ImGui::GetIO();
        uint32_t key = (uint32_t)event.GetKeyCode();
        if (key > 0 && key < 0x1000)
    ...
    {
        ImGuiIO& io = ImGui::GetIO();
        io.AddMouseButtonEvent(PtlToImGuiMouseButtonCode(event.GetMouseButton()), true);
        return false;
    }
    ...
    {
        ImGuiIO& io = ImGui::GetIO();
        io.AddMouseButtonEvent(PtlToImGuiMouseButtonCode(event.GetMouseButton()), false);
        return false;
    }
