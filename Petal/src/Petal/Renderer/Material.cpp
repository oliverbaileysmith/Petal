...
namespace ptl
{
	...
	void PhongMaterial::Bind() const
	{
		...
		m_Shader->UploadUniformBool("u_Material.UsingAmbientDiffuseMap",
			m_UsingAmbientDiffuseMap);
		m_Shader->UploadUniformBool("u_Material.UsingSpecularMap",
			m_UsingSpecularMap);

		if (m_UsingAmbientDiffuseMap)
		{
			m_AmbientDiffuseMap->Bind(m_AmbientDiffuseSlot);
			m_Shader->UploadUniformInt("u_Material.AmbientDiffuseMap",
				m_AmbientDiffuseSlot);
		}
		if (m_UsingSpecularMap)
		{
			m_SpecularMap->Bind(m_SpecularSlot);
			m_Shader->UploadUniformInt("u_Material.SpecularMap",
				m_SpecularSlot);
		}
	}
	...
	void PhongMaterial::AddAmbientDiffuseMap(Ref<Texture2D> ambientDiffuseMap,
		uint32_t slot)
	{
	...
	void PhongMaterial::AddSpecularMap(Ref<Texture2D> specularMap,
		uint32_t slot)
	{
	...
	LampMaterial::LampMaterial(const glm::vec3 &color) : m_Color(color)
	{
